...
class UserController extends Controller
{
    ...
    public function wallet(Request $request)
    {
        ...
        $transaction_data=Transaction::select('order_number','transaction_type','wallet',DB::raw('DATE_FORMAT(created_at, "%d %M %Y") as date'),'username','order_id')->where('user_id',$user_id)->orderBy('id', 'DESC')->paginate(6);
        $getabout = About::where('id','=','1')->first();
        if (Auth::guest()) {
            return view('front.login', compact('cart','getdata','getcategory','getabout'));
        } else {
            return view('front.wallet', compact('getcategory','cart','transaction_data','walletamount','getdata','getabout'));
        }
    }
